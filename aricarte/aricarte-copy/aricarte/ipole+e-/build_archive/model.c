...
double beta_crit_coefficient = 0.5; //ARR:  For electronModel == 4
double constant_beta_e0 = 0.1;  //ARR:  For electronModel == 5
...
//    3 : use mixed TP_OVER_TE (beta model) with fluid temperature for compatibility with other formats, like Ressler.
//    4 : use critical beta model (Anantua)
//    5 : use constant electron beta model (Anantua)
...
void try_set_model_parameter(const char *word, const char *value)
{
  ...
  set_by_word_val(word, value, "sigma_transition", &sigma_transition, TYPE_DBL);
  ...
  set_by_word_val(word, value, "constant_beta_e0_exponent", &constant_beta_e0_exponent, TYPE_DBL);
...
void init_physical_quantities(int n)
{
  ...
  //ARR:  Want typical values of b for normalizing the bias model.  Kind of awkward, since B hasn't been established yet.
  //TODO
    ...
    for (int j = 0; j < N2+2; j++) {
      for (int k = 0; k < N3+2; k++) {
        ...
        if (electronModel == 1) {
          ...
          Thetae_unit = (MP/ME) * (game-1.) * (gamp-1.) / ( (gamp-1.) + (game-1.)*trat );
          data[n]->thetae[i][j][k] = Thetae_unit*data[n]->p[UU][i][j][k]/data[n]->p[KRHO][i][j][k];
        } else if (electronModel == 5) {
          //ARR:  See definition in Anantua et al. (2020).
...
void init_iharm_grid(char *fnam, int dumpidx)
{
  ...
  //ARR:  Don't understand the motivation for this.
  /*
  ...
  */
  ...
  if ( hdf5_exists("weights") ) {
    ...
    Ne_factor = 1. / mu_e;
    hdf5_set_directory("/header/");
    //ELECTRONS = ELECTRONS_TFLUID;  //ARR:  Don't understand the motivation for this.
  }
  ...
  if (electronModel == 0) {
    ...
    fprintf(stderr, "Using critical beta model with mixed tp_over_te and\n");
    fprintf(stderr, "beta_crit_coefficient = %g, beta_crit = %g\n", beta_crit_coefficient, beta_crit);
  } else if (electronModel == 5) {
    fprintf(stderr, "Using constant_beta_e0 model with tp_over_te and \n");
    fprintf(stderr, "constant_beta_e0 = %g\n", constant_beta_e0);
  } else {
...
void output_hdf5()
{
  ...
  hdf5_write_single_val(&constant_beta_e0_exponent, "constant_beta_e0_exponent", H5T_IEEE_F64LE);
