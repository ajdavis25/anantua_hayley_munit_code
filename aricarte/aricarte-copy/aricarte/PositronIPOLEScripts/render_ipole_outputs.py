...
def process_dump_file(dump_file):
    ...
    for model_name, settings in model_settings.items():
        ...
        U0_star, _ = secant_for_flux(
            0.0, p.M_base, p.A0 + p.S0 * p.M_base, None, 1,
            simFile, nameBase, electronModel, sigma_transition
        )
        U1_star, _ = secant_for_flux(
            1.0, p.M_base, p.A0 + p.S0 * p.M_base / 3.0, None, 2,
            simFile, nameBase, electronModel, sigma_transition
        )
