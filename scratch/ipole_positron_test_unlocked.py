...
def run_from_csv(
    ...
    if row_index is not None:
        logging.info(f"running single row index: {row_index}")
        row = df.iloc[int(row_index)]
        df = pd.DataFrame([row]) # single-row DataFrame

    for _, row in df.iterrows():
        if pd.isna(row["Model"]) or pd.isna(row["Munit"]):
            logging.warning(f"skipping row with missing Model or Munit: {row.to_dict()}")
            continue
        ...
        prefix_map = {"MAD": "Ma", "SANE": "Sa"}
        prefix = prefix_map.get(mad_sane)
        if prefix is None:
